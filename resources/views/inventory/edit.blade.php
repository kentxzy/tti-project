    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 class="text-gray-800 font-medium">Edit Product</h3>
                ...
                <form action="{{ route('inventory.update', $inventory->id) }}" method="POST">
                    ...
                    <div>
                        ...
                        <input type="text" name="name"
                            value="{{ old('name', $inventory->product->name) }}"
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select name="category"
                                ...
                                @foreach(['CPU', 'GPU', 'Motherboard', 'RAM', 'Storage', 'PSU', 'Case', 'Laptop', 'Desktop', 'Monitor', 'Peripherals', 'Networking'] as $cat)
                                    <option value="{{ $cat }}"
                                        {{ old('category', $inventory->product->category) == $cat ? 'selected' : '' }}>
                                        {{ $cat }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="price"
                                value="{{ old('price', $inventory->product->price) }}"
                    ...
                    <div class="flex items-center gap-2 p-3 bg-gray-50 rounded-lg border border-gray-100">
                        <p class="text-xs text-gray-500">Current stock status:</p>
                        @php
                        ...
                        @endphp
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs border {{ $badge }}">
                            {{ $label }} ({{ $stock }} units)
                        </span>
                    </div>
